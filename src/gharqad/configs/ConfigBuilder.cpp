...
void BuildConfigSingBox(const std::shared_ptr<BuildConfigStatus> &status) {
  ...
  */
  ...
  // hydra: the core transport must never re-enter the tun (infinite loop),
  // and tcp must stay off the udp-only socks bridge
  if (status->ent->type == "hydra") {
#ifdef Q_OS_WIN
...
#endif
    routeChain->Rules << RouteRule::get_processPath_direct_rule(core);
    auto tcpRule = std::make_shared<RouteRule>();
    tcpRule->name = "AutoGenerated hydra tcp direct";
    tcpRule->action = "route";
    tcpRule->outboundID = -2; // direct
    tcpRule->network = "tcp";
    routeChain->Rules << tcpRule;
    // process lookup fails for some udp flows (elevated core), so the server
    // ip itself is pinned to direct — routing cannot send it back into the tun
    if (IsIpAddress(status->ent->serverAddress)) {
      auto ipRule = std::make_shared<RouteRule>();
      ipRule->name = "AutoGenerated hydra server direct";
      ipRule->action = "route";
      ipRule->outboundID = -2;
      ipRule->ip_cidr = {status->ent->serverAddress + "/32"};
      routeChain->Rules << ipRule;
      // os-level: the server ip never enters the tun adapter at all
      status->tunServerExcludes << status->ent->serverAddress;
    }
  }
...
skip_multiple_jobs:
  ...
  if ((dataStore->spmode_vpn && !status->forTest) || blockAll) {
    status->inbounds += BuildTunInbound(directIPSets,
                                        directIPCIDRs + status->tunServerExcludes);
  }
  ...
  // hydra: tcp-based dns (dot/doh/tcp) cannot cross the udp-only stdio
  // bridge, so the remote resolver is forced to plain udp
  bool hydraNeedsUdpDns = status->ent->type == "hydra";
  ...
  if (blockAll) {
    ...
    if (hydraNeedsUdpDns) {
      auto dnsType = remoteDnsObj["type"].toString();
      if (dnsType != "udp" && dnsType != "quic") {
        // re-resolve the same host over plain udp on the standard port
        auto server = remoteDnsObj["server"].toString();
        remoteDnsObj = QJsonObject{{"type", "udp"}, {"server", server}};
        remoteDnsObj["server_port"] = 53;
      }
    }
