...
skip_multiple_jobs:
  ...
  if (!status->forTest) {
    ...
    // hydra: the core's own packets (dns/stun/hop channels) and all tcp must
    // never enter the tunnel — the ip_cidr rule kills the loop even when
    // process lookup fails, and it must precede the dns hijack rule
    if (status->ent != nullptr && status->ent->type == "hydra") {
      ...
      if (IsIpAddress(status->ent->serverAddress)) {
        hydraRules += QJsonObject{{"outbound", "direct"},
                                  {"ip_cidr", QJsonArray{status->ent->serverAddress + "/32"}}};
      }
